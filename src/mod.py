...
from . import feat  # Import feat module for region mapping
...
class DumEnc:
    def __init__(self, dtype=int):
        self.OHE = OneHotEncoder(
            dtype=dtype, handle_unknown="ignore", sparse_output=False
        )
        self.var_names = None
    ...
    def transform(self, X):
        res = pd.DataFrame(self.OHE.transform(X), columns=self.var_names, index=X.index)
        return res


def dummy_stats(values, begin_date):
    vdate = pd.to_datetime(values)
    year = vdate.dt.year
...
class DateEnc:
    def __init__(
        self, begin="1/1/2015", dummy=True, dum_types=["days", "weekday", "month"]
    ):
    ...
    def transform(self, X):
        ...
        if self.dummy:
            ...
            for v in vars:
                dd, year_cos, year_sin, week_cos, week_sin = circle_stats(
                    X[v], self.begin
                )
...
class SimpleOrdEnc:
    def __init__(self, dtype=int, unknown_value=-1, lim_k=None, lim_count=None):
...
class RegMod:
    ...
    def met_eval(
        ...
        if pr:
            print(
                mpd[["AvgError", "midwest", "northeast", "south", "west"]].describe().T
            )
        if ret:
...
class CatMod:
    def __init__(
        ...
        mod=CatBoostClassifier(
            iterations=100, depth=5, allow_writing_files=False, verbose=False
        ),
    ):
        self.fe = FeatureEngine(
            ord_vars=ord_vars,
...
def rmse_region(
    data, pred="pred", true="severity", region="region", scale=False, ret=False
):
    dc = data[[region, true, pred]].copy()
    if scale:
        dc[pred] = strat(dc[pred])
    dc[pred] = dc[pred].round().astype(int).clip(1, 5)
    # Use centralized region mapping from feat.py
    dc[region] = dc[region].replace(feat.reg_ord_reverse)
...
def check_day(current, day="sub_2023_01_31.csv"):
    old = pd.read_csv(rf"./submissions/{day}")
    dstr = f"dif_{day[4:-4]}"
