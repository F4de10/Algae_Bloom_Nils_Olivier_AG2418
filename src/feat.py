...
reg_ord = {"west": 4, "midwest": 3, "south": 2, "northeast": 1}

# Reverse mapping for converting numbers back to region names
reg_ord_reverse = {v: k for k, v in reg_ord.items()}
...
def get_data(data_type="train", db_str=db, split_pred=False):
    ...
    if split_pred:
        pred_test = pd.read_sql(
            "SELECT uid, pred AS split_pred FROM split_pred", con=db_con
        )
...
# convert numeric regions back to strings
def region_to_string(data, rstr="region"):
    """Convert numeric region codes back to string names"""
    data[rstr] = data[rstr].replace(reg_ord_reverse)
